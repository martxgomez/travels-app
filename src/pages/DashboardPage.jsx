...
function Dashboardpage({ travels, favorites, addFavorite }) {
  ...
  const filteredTravels = travels.filter((travel) => {
    const matchesDestination = travel.destination
      .toLowerCase()
      .includes(search.toLowerCase()); //first filter - destinations that match what we type in the search bar
  ...
  const maxAvailablePrice = travels.length
    ? Math.max(...travels.map((travel) => travel.price))
    : 0;
  return (
    <div className="dashboard-container">
      ...
      {showPriceError && (
        <p style={{ color: "red" }}>
          Price must be a positive number, maximum allowed price is{" "}
          {maxAvailablePrice} €.
        </p>
      )}
